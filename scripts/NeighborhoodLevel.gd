...
@onready var battle_trigger: Area2D = $NeighborhoodBattleTrigger
...
func _process(_delta: float) -> void:
	can_start_battle = _is_player_near_battle()
	interact_panel.visible = can_start_battle and not battle_completed

	if can_start_battle and not battle_completed and Input.is_action_pressed("interact"):
		_start_battle()
...
func _configure_level_bounds() -> void:
	...
	battle_trigger.position = Vector2(level_width - 260.0, FLOOR_TOP_Y - 100.0)
...
func _refresh_ui() -> void:
	battle_completed = GameState.is_objective_completed("first_battle_won")
	if battle_completed:
		objective_label.text = "Barrio: batalla ganada\nCiudad desbloqueada. Presiona M para abrir el mapa."
	else:
		objective_label.text = "Barrio\nAvanza hasta la cancha y acepta la primera batalla importante.\nPresiona M para abrir el mapa."

	interact_label.text = "Presiona %s para empezar la batalla del barrio" % SettingsManager.get_action_label("interact")


func _is_player_near_battle() -> bool:
	if player == null or battle_trigger == null:
		return false

	return player.global_position.distance_to(battle_trigger.global_position) <= BATTLE_DISTANCE
...
func _on_neighborhood_battle_trigger_body_entered(body: Node2D) -> void:
	if body == player:
		can_start_battle = true


func _on_neighborhood_battle_trigger_body_exited(body: Node2D) -> void:
	if body == player:
